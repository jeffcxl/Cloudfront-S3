...
resource "aws_s3_bucket" "static_web" {
  bucket        = "jeffstaticweb" #Name of your bucket
  force_destroy = true
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  enabled             = true
  comment             = "Jeff staticweb using cloudfront" #Description
  default_root_object = "index.html"

  aliases = ["jeffstaticweb.sctp-sandbox.com"] #Cloudfront alternate domain name (Same as Route53 record)
...
resource "aws_cloudfront_origin_access_control" "oac" {
  name                              = "jeff-cloudfront-oac"    #e.g. <yourname>-cloudfront-oac
...
resource "aws_route53_record" "www" {
  ...
  name    = "jeffstaticweb"  #Your domain prefix. <this-value>.sctp-sandbox.com
  type    = "A"
